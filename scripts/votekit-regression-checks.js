...
assert(resultsRoute.includes('new Set<string>(voteData.choices)'), 'multiple-choice tally counts each option at most once per ballot');
...
assert(resultsRoute.includes('condorcetResult.tiedCandidates'), 'results API exposes Condorcet/Schulze tie information');
