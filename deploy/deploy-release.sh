...
if [[ ! -e "$RELEASE_DIR" ]]; then
  ...
  FIREBASE_PUBLIC_ENV=()
  for KEY in NEXT_PUBLIC_FIREBASE_API_KEY NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN NEXT_PUBLIC_FIREBASE_PROJECT_ID; do
    VALUE="$(set +u; source /etc/votekit/votekit.env; printf '%s' "${!KEY:-}")"
    if [[ -n "$VALUE" ]]; then FIREBASE_PUBLIC_ENV+=("$KEY=$VALUE"); fi
  done
  VOTEKIT_RELEASE_RUN=("${VOTEKIT_RUN[@]}" "${FIREBASE_PUBLIC_ENV[@]}" DATABASE_PATH="$DATABASE_PATH" VOTEKIT_RELEASE="$VOTEKIT_RELEASE" VOTEKIT_ENCRYPTED_BALLOTS_ENABLED="$VOTEKIT_ENCRYPTED_BALLOTS_ENABLED")
