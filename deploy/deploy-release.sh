...
if [[ ! -e "$RELEASE_DIR" ]]; then
  ...
  runuser -u votekit -- env HOME="$HOME" NPM_CONFIG_CACHE="$NPM_CONFIG_CACHE" npm --prefix "$INCOMING" ci

  export DATABASE_PATH=:memory:
  export VOTEKIT_RELEASE="$RELEASE_SHA"
  export VOTEKIT_ENCRYPTED_BALLOTS_ENABLED=false
  runuser -u votekit -- env HOME="$HOME" NPM_CONFIG_CACHE="$NPM_CONFIG_CACHE" DATABASE_PATH="$DATABASE_PATH" VOTEKIT_RELEASE="$VOTEKIT_RELEASE" VOTEKIT_ENCRYPTED_BALLOTS_ENABLED="$VOTEKIT_ENCRYPTED_BALLOTS_ENABLED" npm --prefix "$INCOMING" run lint
  runuser -u votekit -- env HOME="$HOME" NPM_CONFIG_CACHE="$NPM_CONFIG_CACHE" DATABASE_PATH="$DATABASE_PATH" VOTEKIT_RELEASE="$VOTEKIT_RELEASE" VOTEKIT_ENCRYPTED_BALLOTS_ENABLED="$VOTEKIT_ENCRYPTED_BALLOTS_ENABLED" npm --prefix "$INCOMING" test
  runuser -u votekit -- env HOME="$HOME" NPM_CONFIG_CACHE="$NPM_CONFIG_CACHE" DATABASE_PATH="$DATABASE_PATH" VOTEKIT_RELEASE="$VOTEKIT_RELEASE" VOTEKIT_ENCRYPTED_BALLOTS_ENABLED="$VOTEKIT_ENCRYPTED_BALLOTS_ENABLED" npm --prefix "$INCOMING" run regression-check
  runuser -u votekit -- env HOME="$HOME" NPM_CONFIG_CACHE="$NPM_CONFIG_CACHE" DATABASE_PATH="$DATABASE_PATH" VOTEKIT_RELEASE="$VOTEKIT_RELEASE" VOTEKIT_ENCRYPTED_BALLOTS_ENABLED="$VOTEKIT_ENCRYPTED_BALLOTS_ENABLED" npm --prefix "$INCOMING" run type-check
  runuser -u votekit -- env HOME="$HOME" NPM_CONFIG_CACHE="$NPM_CONFIG_CACHE" DATABASE_PATH="$DATABASE_PATH" VOTEKIT_RELEASE="$VOTEKIT_RELEASE" VOTEKIT_ENCRYPTED_BALLOTS_ENABLED="$VOTEKIT_ENCRYPTED_BALLOTS_ENABLED" npm --prefix "$INCOMING" run build
  unset DATABASE_PATH
